			...
			if ( $query->is_archive() && ! $query->is_tax() && ! $query->is_category() && ! $query->is_tag() ) {
				...
				if ( ! empty( $last_category ) ) {
					...
					foreach ( $taxonomy_terms as $index => $taxonomy ) {
						...
						echo '<li ' . wp_kses_post( $this->get_render_attribute_string( 'breadcrumbs-item-post-cpt' ) ) . '"><a ' . wp_kses_post( $this->get_render_attribute_string( 'breadcrumbs-item-post-cpt-crumb' ) ) . '>' . esc_attr( $cat_name ) . '</a></li>';

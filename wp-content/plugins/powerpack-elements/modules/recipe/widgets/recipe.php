...
class Recipe extends Powerpack_Widget {
	 ...
	 * @access protected
	 */
	protected function render_recipe_details_icon( $icon ) {
		...
		<div class="pp-recipe-container" itemscope="" itemtype="http://schema.org/Recipe">
			<div class="pp-recipe-header">
				...
				<div class="pp-recipe-header-content">
					<?php if ( ! empty( $settings['recipe_name'] ) ) { ?>
						<h2 <?php echo wp_kses_post( $this->get_render_attribute_string( 'recipe_name' ) ); ?>>
			...
			<div class="pp-recipe-details pp-recipe-section">
				<ul class="pp-recipe-detail-list">
					...
					<li itemprop="prepTime" content="PT<?php echo esc_attr( $settings['prep_time'] ); ?>M">
					...
					<li itemprop="cookTime" content="PT3<?php echo esc_attr( $settings['cook_time'] ); ?>M">
					...
					<li itemprop="totalTime" content="PT<?php echo esc_attr( $settings['total_time'] ); ?>M">
					...
					foreach ( $settings['recipe_ingredients'] as $index => $item ) :
						...
						if ( $item['recipe_ingredient'] ) : ?>
								<li class="pp-recipe-ingredient">
									...
									<?php } ?>
									<span itemprop="recipeIngredient" <?php echo wp_kses_post( $this->get_render_attribute_string( $ingredient_key ) ); ?>>
				...
				<ol class="pp-recipe-instructions-list">
					...
					foreach ( $settings['recipe_instructions'] as $index => $item ) :
						...
						if ( $item['recipe_instruction'] ) : ?>
								<li itemprop="recipeInstructions" <?php echo wp_kses_post( $this->get_render_attribute_string( $instruction_key ) ); ?>>

...
class Toggle extends Powerpack_Widget {
	...
	protected function render_toggle( $toggle_position = 'before' ) {
		...
		?>
		<div <?php echo wp_kses_post( $this->get_render_attribute_string( 'toggle-switch-wrap' ) ); ?>>
			<div class="pp-toggle-switch-inner">
				<?php if ( $settings['primary_label'] ) { ?>
					<div <?php echo wp_kses_post( $this->get_render_attribute_string( 'primary-label' ) ); ?>>
						<?php echo esc_attr( $settings['primary_label'] ); ?>
					</div>
				<?php } ?>
				<div <?php echo wp_kses_post( $this->get_render_attribute_string( 'toggle-switch-container' ) ); ?>>
				...
				<?php if ( $settings['secondary_label'] ) { ?>
					<div <?php echo wp_kses_post( $this->get_render_attribute_string( 'secondary-label' ) ); ?>>
	...
	protected function render() {
		...
		?>
		<div <?php echo wp_kses_post( $this->get_render_attribute_string( 'toggle-container' ) ); ?>>
			<?php
			if ( 'before' === $settings['toggle_position'] || 'before-after' === $settings['toggle_position'] ) {
				$this->render_toggle( 'before' );
			}
			?>
			<div <?php echo wp_kses_post( $this->get_render_attribute_string( 'toggle-content-wrap' ) ); ?>>
				<div <?php echo wp_kses_post( $this->get_render_attribute_string( 'toggle-section-primary' ) ); ?>>
					<?php
					if ( 'content' === $settings['primary_content_type'] ) {
						...
						if ( ! empty( $settings['primary_templates'] ) ) {
							...
							echo pp_get_elementor()->frontend->get_builder_content_for_display( $primary_template_id ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
						}
					}
					?>
				</div>
				<div <?php echo wp_kses_post( $this->get_render_attribute_string( 'toggle-section-secondary' ) ); ?>>
					<?php
					if ( 'content' === $settings['secondary_content_type'] ) {
						...
						if ( ! empty( $settings['secondary_templates'] ) ) {
							...
							echo pp_get_elementor()->frontend->get_builder_content_for_display( $secondary_template_id ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
						}
					}
					?>
				</div>

...
$add_to_wishlist_text = __( 'Add to Wishlist', 'powerpack' );
...
<li <?php post_class(); ?>>
	<div class="pp-woo-product-wrapper">
		...
		if ( '' !== $quick_view_type ) {
			...
			echo '<div class="pp-action-item-wrap pp-quick-view-btn" data-product_id="' . esc_attr( $post_id ) . '">';
		...
		if ( 'yes' === $settings['show_add_cart'] ) {
			...
			echo '<a href=' . esc_url( $product->add_to_cart_url() ) . ' class="pp-action-item-wrap pp-cart-section ' . esc_attr( $cart_class ) . ' product_type_' . esc_attr( $product->get_type() ) . '" data-quantity="1" data-product_id="' . esc_attr( $post_id ) . '">';
		...
		if ( class_exists( 'YITH_WCWL' ) && 'yes' === $settings['show_add_to_wishlist'] ) {
			...
			echo do_shortcode( '[yith_wcwl_add_to_wishlist label="" link_classes="pp-action-item" already_in_wishslist_text="" product_added_text="" browse_wishlist_text=""]' );

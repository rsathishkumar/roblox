...
class CustomUpload extends \ElementorPro\Modules\Forms\Fields\Field_Base {
  ...
  public function get_name() {
    return __( 'File Upload Button', 'elementor-pro' );
  }
  ...
  public function update_controls( $widget ) {
    ...
    $field_controls = [
      ...
      'allow_multiple_upload2' => [
        'name' => 'allow_multiple_upload2',
  ...
  public function render( $item, $item_index, $form ) {
    $form->add_render_attribute( 'input' . $item_index, 'class', 'elementor-upload-field, elementor-button' );
    $form->add_render_attribute( 'input' . $item_index, 'type', 'file', true );

    if ( ! empty( $item['allow_multiple_upload2'] ) ) {
    ...
    if ( ! empty( $item['file_sizes2'] ) ) {
      $form->add_render_attribute(
        'input' . $item_index,
        [
          'data-maxsize' => $item['file_sizes2'],  //MB
    ...
    <div class="fileUpload"><div class="elementor-button e-form__buttons__wrapper__button-next elementor-size-'.$item['input_size'].' "><span class="elementor-button-text elementor-size-'.$item['input_size'].'">Upload</span></div>
<input ' . $form->get_render_attribute_string( 'input' . $item_index ) . ' /><div id="uploadFile"></div></div></div>';
  }
  ...
  private function is_file_size_valid( $field, $file ) {
    $allowed_size = ( ! empty( $field['file_sizes2'] ) ) ? $field['file_sizes2'] : wp_max_upload_size() / pow( 1024, 2 );
  ...
  private function is_file_type_valid( $field, $file ) {
    ...
    if ( empty( $field['file_types2'] ) ) {
      $field['file_types2'] = 'jpg,jpeg,png,gif,pdf,doc,docx,ppt,pptx,odt,avi,ogg,m4a,mov,mp3,mp4,mpg,wav,wmv';
    }

    $file_extension = pathinfo( $file['name'], PATHINFO_EXTENSION );
    $file_types_meta = explode( ',', $field['file_types2'] );
  ...
  public function validation( $field, Classes\Form_Record $record, Classes\Ajax_Handler $ajax_handler ) {
    ...
    if ( ! empty( $field['max_files2'] ) ) {
      if ( count( $_FILES['form_fields'][ $id ] ) > $field['max_files2'] ) {
        $error_message = sprintf(
        /* translators: %d: The number of allowed files. */
          _n( 'You can upload only %d file.', 'You can upload up to %d files.', intval( $field['max_files2'] ), 'elementor-pro' ),
          intval( $field['max_files2'] )
        );
  ...
  public function __construct() {
    ...
    wp_enqueue_style( "custom-upload-field", CUSTOM_WIDGET_URL . 'css/file-upload-field.css' );
    wp_enqueue_script( "custom-upload-field-js", CUSTOM_WIDGET_URL . 'js/custom_upload_field.js' , ['jquery-core']);
  }
